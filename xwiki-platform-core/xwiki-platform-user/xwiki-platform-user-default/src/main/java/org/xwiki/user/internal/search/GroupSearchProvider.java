...
/**
 * Implementation of {@link ActorsSearchProvider} for the wiki group search.
 */
@Component
@Singleton
@Named("group")
public class GroupSearchProvider implements ActorsSearchProvider
{
    private static final String SEARCH_QUERY =
        "from doc.object(XWiki.XWikiGroups) as groups "
            + "where lower(doc.name) like :input "
            + "and doc.fullName <> 'XWiki.XWikiGroupTemplate' "
    ...
    public List<ActorSearchResult> search(WikiReference wiki, String input)
    {
        ...
        try {
            ...
            this.logger.warn("Failed to create the group search query. Cause: [{}]", getRootCauseMessage(e));
            return emptyList();
        }
    }
}

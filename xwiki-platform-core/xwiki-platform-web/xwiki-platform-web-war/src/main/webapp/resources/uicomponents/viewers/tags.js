        ...
        new Ajax.Request(
          item.readAttribute('href').replace(/&xredirect=.+$/, "&ajax=1"),
          {
            onCreate : function () {
              ...
              item.disabled = true;
              item.notification = new XWiki.widgets.Notification("$escapetool.javascript($services.localization.render('core.tags.deleting'))", "inprogress");
            },
            onSuccess : function () {
          ...
          new Ajax.Request(
            item.readAttribute('href').replace(/#.+$/, "&ajax=1&xpage=documentTags"),
            {
              onCreate : function () {
                // ignore "cascade" clicks
                item.disabled = true;
                item.notification = new XWiki.widgets.Notification("$escapetool.javascript($services.localization.render('core.tags.fetchform'))", "inprogress");
              },
        ...
        new Ajax.Request(
          form.action.replace(/&xredirect=.+$/, '&ajax=1&tag=') + encodeURIComponent(form.tag.value),
          {
            onCreate : function () {
              // ignore "cascade" clicks
              form.disable();
              form.notification = new XWiki.widgets.Notification("$escapetool.javascript($services.localization.render('core.tags.adding'))", "inprogress");
            },
            onSuccess : function (response) {
